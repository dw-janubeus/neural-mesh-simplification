...
from ..data.optimized_dataset import (
...
class OptimizedTrainer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.use_mixed_precision = (
            config.get("mixed_precision", True) and torch.cuda.is_available()
        )
        ...
        self.effective_batch_size = (
            config["training"]["batch_size"] * self.gradient_accumulation_steps
        )
        logger.info(
            f"Effective batch size: {self.effective_batch_size} (batch_size={config['training']['batch_size']}, accumulation_steps={self.gradient_accumulation_steps})"
        )
    ...
    def _setup_optimizer_and_scheduler(self):
        ...
        logger.info(
            f"Using {optimizer_type.upper()} optimizer with {scheduler_type} scheduler"
        )
    ...
    def _prepare_optimized_data_loaders(self):
        ...
        data_dir = self.config["data"].get(
            "optimized_data_dir", self.config["data"]["data_dir"]
        )
        ...
        if not os.path.exists(os.path.join(data_dir, "metadata")):
            ...
            logger.warning(
                "Falling back to original dataset - performance will be suboptimal"
            )
        ...
        if self.config["training"].get("use_augmentation", True):
            augmentation = DataAugmentation(
                rotation_prob=0.5, noise_prob=0.3, noise_std=0.01
            )
        ...
        dataset = OptimizedMeshSimplificationDataset(
            ...
            subset_size=self.config.get(
                "debug_subset_size"
            ),  # For debugging with smaller dataset
        )
        ...
        if val_size == 0:
            raise ValueError(
                "Validation set is empty. Increase dataset size or reduce val_split."
            )
        ...
        logger.info(
            f"Data loader settings: {num_workers} workers, pin_memory={pin_memory}"
        )
    ...
    def train(self):
        ...
        if self.monitor_resources:
            ...
            self.resource_monitor = ImprovedResourceMonitor(
                main_pid=main_pid, update_interval=1.0, show_detailed=True
            )
        ...
        try:
            ...
            if self.training_stats["epoch_times"]:
                avg_epoch_time = sum(self.training_stats["epoch_times"]) / len(
                    self.training_stats["epoch_times"]
                )
    ...
    def _train_one_epoch(self, epoch: int) -> float:
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if self.resource_monitor:
                self.resource_monitor.end_batch(
                    batch.num_graphs
                ...
                if batch_idx % 10 == 0:  # Update every 10 batches
                    current_lr = self.optimizer.param_groups[0]["lr"]
                    current_loss = (
                        running_loss / num_batches if num_batches > 0 else 0.0
                    )
                    self.resource_monitor.update_training_progress(
                        epoch=epoch + 1,
                        batch=batch_idx,
    ...
    def get_training_summary(self) -> Dict:
        ...
        summary = {
            ...
            "avg_epoch_time": sum(self.training_stats["epoch_times"])
            / len(self.training_stats["epoch_times"]),
            "best_val_loss": self.best_val_loss,
            "final_lr": (
                self.training_stats["learning_rates"][-1]
